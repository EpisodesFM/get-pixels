...
function handleGIF(data, maxGifFrames, cb) {
  var reader;
  try {
  ...
  if (reader.numFrames() > 0) {
    var numFrames =
      maxGifFrames > 0
        ...
        : reader.numFrames();
    var nshape = [numFrames, reader.height, reader.width, 4];
    try {
...
function doParse(mimeType, data, cb, options) {
  switch (mimeType) {
    case 'image/png':
      handlePNG(data, cb);
      break;
    ...
    case 'image/gif':
      handleGIF(new Uint8Array(data), options.maxGifFrames, cb);
      break;
...
module.exports = function getPixels(
  url,
  type,
  cb,
  options = { maxGifFrames: -1 }
) {
  if (!cb) {
    cb = type;
    type = '';
  ...
  if (Buffer.isBuffer(url)) {
    ...
    fetch(url)
      ...
      .then((body) => {
        doParse(contentType, body, cb, options);
      })
      .catch((err) => {
        cb(err);
      });
